...
from backend.pipeline.nodes.approve_criteria import approve_criteria
from backend.pipeline.utils import format_criteria
...
def test_approve_all_keywords(keyword):
    with patch("backend.pipeline.nodes.approve_criteria.interrupt", return_value={"feedback": keyword}):
...
def test_approve_strips_whitespace():
    with patch("backend.pipeline.nodes.approve_criteria.interrupt", return_value={"feedback": "  ok  "}):
        result = approve_criteria(SAMPLE_STATE)
...
def test_approve_missing_feedback_key_defaults_to_ok():
    with patch("backend.pipeline.nodes.approve_criteria.interrupt", return_value={}):
        result = approve_criteria(SAMPLE_STATE)
...
def test_revision_loops_back():
    ...
    with patch("backend.pipeline.nodes.approve_criteria.interrupt", return_value={"feedback": "add leadership criterion"}), \
         patch("backend.pipeline.nodes.approve_criteria.llm", _make_llm_mock(revised)):
        result = approve_criteria(SAMPLE_STATE)
...
def test_revision_updates_scoring_criteria():
    ...
    with patch("backend.pipeline.nodes.approve_criteria.interrupt", return_value={"feedback": "needs more experience"}), \
         patch("backend.pipeline.nodes.approve_criteria.llm", _make_llm_mock(revised)):
        result = approve_criteria(SAMPLE_STATE)
...
def test_empty_feedback_goes_to_revision():
    ...
    with patch("backend.pipeline.nodes.approve_criteria.interrupt", return_value={"feedback": ""}), \
         patch("backend.pipeline.nodes.approve_criteria.llm", _make_llm_mock(revised)):
        result = approve_criteria(SAMPLE_STATE)
...
def test_revision_with_empty_scoring_criteria():
    state = {**SAMPLE_STATE, "scoring_criteria": []}
    revised = [{"name": "Python", "description": "3+ years"}]
    with patch("backend.pipeline.nodes.approve_criteria.interrupt", return_value={"feedback": "add python criterion"}), \
         patch("backend.pipeline.nodes.approve_criteria.llm", _make_llm_mock(revised)):
        result = approve_criteria(state)
